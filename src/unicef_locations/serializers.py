...
class CartoDBTableSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = CartoDBTable
        fields = ('id', 'domain', 'api_key', 'table_name', 'display_name',
                  'pcode_col', 'color', 'location_type', 'name_col')
...
class LocationSerializer(serializers.ModelSerializer):
    ...
    location_type_admin_level = serializers.IntegerField(
        source='gateway.admin_level')
    geo_point = serializers.SerializerMethodField()
    name = serializers.SerializerMethodField()

    def get_geo_point(self, obj):
        return str(obj.geo_point)

    def get_name(self, obj):
        return f'{obj.name} [{obj.gateway.name} - {obj.p_code}]'
